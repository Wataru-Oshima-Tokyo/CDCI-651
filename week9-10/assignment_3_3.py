...
"""
Answer:
What I observed is that there is an obvious difference among the three methods. For the random methods, the infection spreads until 
the probability hits 50% but for the neighbor method, it seems like more efficient to prevent from being infected since enve though
the probability is only 30%, the infection rate is relateively lower than the random one. However, the method is still not perfect 
since the inferction rate is dramitically increased at the certain period wihch is around 6th iteration. For the high degree methods, 
it is the best solution to stop spreading infection since even though the porability is only 10%, the infection rate is below 20%.
But realistically, we are not sure who has the high degree so it would be difficult to use the methods. Instead, the neighbor method
which is the based on the concetpt where your firends has more connections, it is more realistic to be used.
...
def _simSIR(G, immunization, p, j):
    global _time_interval, _no_imm
    print("started funciton")
...
if __name__ == "__main__":
    ...
    # # pool.starmap(simSIR, [(n,m0,m,arpha[0]),(n,m0,m,arpha[1]),(n,m0,m,arpha[2])]) # make a graph simultenously
